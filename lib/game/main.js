...
.defines(function(){
  ...
  GameInfo = new function () {
   ...
   this.price  = {
     ...
     rooster: 150,
     feed: 600,
     level: [ 0, 5000, 18500, 41500, 84400, 151900, 264000, 414000, 627900, 627900 ],
     basket: [ 0, 3000, 16500, 37200, 66900, 107400, 172300, 277300, 0 ],
   };
 };
  ...
  MyGame = ig.Game.extend({
    ...
    init: function() {
      // 레벨 불러오기
      var level_string = 'LevelLevel'+GameInfo.currentLevel;
    ...
    update: function() {
      ...
      if ( ig.game.getEntitiesByType(EntityHen).length === 0 && this.gameStarted == true ) {
    ...
    setStage: function(roosters, hens) {
      ...
      ig.game.spawnEntity( Button, 10, 530, {
        ...
        pressedUp: function() {
          ...
          if ( GameInfo.wallet >= GameInfo.price.level[GameInfo.currentLevel] && ig.game.gameStarted == true ) {
            GameInfo.wallet -= GameInfo.price.level[GameInfo.currentLevel];
      ...
      ig.game.spawnEntity( Button, 100, 530, {
        ...
        pressedUp: function() {
          if ( GameInfo.wallet >= GameInfo.price.basket[GameInfo.basket_level+1] && ig.game.gameStarted == true ) {
      ...
      ig.game.spawnEntity( Button, 370, 530, {
        ...
        pressedUp: function() {
          if ( GameInfo.wallet >= GameInfo.price.feed && GameInfo.quality <= 700-(GameInfo.feed) && ig.game.gameStarted == true  ) {
      ...
      ig.game.spawnEntity( Button, 550, 530, {
       ...
       // 테스트용 돈벌기
      ig.game.spawnEntity( Button, 550, 450, {
        font: new ig.Font( 'media/apple.white.font.png' ),
        text: [ 'MONEY' ],
        textPos: { x: 15, y: 5 },
        textAlign: ig.Font.ALIGN.LEFT,
    ...
    draw: function() {
      ...
      this.smallFont.draw(GameInfo.price.basket[GameInfo.basket_level+1], 123, 615);
      ...
      this.smallFont.draw(GameInfo.price.level[GameInfo.currentLevel], 33, 615);

      // 닭 품질
      this.font.draw(GameInfo.quality_level + '/' + GameInfo.quality, 370, 495);
  ...
  MyTitle = ig.Game.extend({
    ...
    draw: function() {
      ...
      var startText = 'Welcome to EGG IMPACT!';
      this.font.draw( startText, cx, 300, ig.Font.ALIGN.CENTER);
